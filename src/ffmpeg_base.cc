...
int FFMPEG_Base::open_codec_context(AVCodecContext **avctx, int stream_idx, AVFormatContext *fmt_ctx, AVMediaType type, const char *filename) const
{
    ...
    if (ret < 0)
    {
        ffmpegfs_error("%s * Failed to open %s input codec for stream #%u (error '%s').", filename, get_media_type_string(type), input_stream->index, ffmpeg_geterror(ret).c_str());
        return ret;
    }

    ffmpegfs_debug("%s * Opened %s input codec for stream #%u.", filename, get_codec_name(codec_id), input_stream->index);
...
{
    AVCodecParameters *par_dst = output_stream->codecpar;
    ...
    ret = avcodec_parameters_to_context(output_codec_ctx, input_stream->codecpar);
    if (ret >= 0)
        ret = av_opt_set_dict(output_codec_ctx, &opts);
    if (ret < 0) {
        av_log(NULL, AV_LOG_FATAL,
               "Error setting up codec context options.\n");
        return ret;
    }
    avcodec_parameters_from_context(par_src, output_codec_ctx);
    ...
    output_stream->avg_frame_rate = ost->frame_rate;

    ret = avformat_transfer_internal_stream_timing_info(of->ctx->oformat, output_stream, input_stream, copy_tb);
    if (ret < 0)
        return ret;

    // copy timebase while removing common factors
    if (output_stream->time_base.num <= 0 || output_stream->time_base.den <= 0)
        output_stream->time_base = av_add_q(av_stream_get_codec_timebase(output_stream), (AVRational){0, 1});

    // copy estimated duration as a hint to the muxer
    if (output_stream->duration <= 0 && input_stream->duration > 0)
        output_stream->duration = av_rescale_q(input_stream->duration, input_stream->time_base, output_stream->time_base);

    // copy disposition
    output_stream->disposition = input_stream->disposition;

    if (input_stream->nb_side_data) {
        for (i = 0; i < input_stream->nb_side_data; i++) {
            const AVPacketSideData *sd_src = &input_stream->side_data[i];
            uint8_t *dst_data;

            dst_data = av_stream_new_side_data(output_stream, sd_src->type, sd_src->size);
            if (!dst_data)
                return AVERROR(ENOMEM);
    ...
    if (ost->rotate_overridden) {
        uint8_t *sd = av_stream_new_side_data(output_stream, AV_PKT_DATA_DISPLAYMATRIX,
                                              sizeof(int32_t) * 9);
        if (sd)
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        else if (input_stream->sample_aspect_ratio.num)
            sar = input_stream->sample_aspect_ratio;
        else
            sar = par_src->sample_aspect_ratio;
        output_stream->sample_aspect_ratio = par_dst->sample_aspect_ratio = sar;
        output_stream->avg_frame_rate = input_stream->avg_frame_rate;
        output_stream->r_frame_rate = input_stream->r_frame_rate;
        break;
    }

    ost->mux_timebase = input_stream->time_base;
