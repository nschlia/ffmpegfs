...
static bool create_bluray_virtualfile(BLURAY *bd, const BLURAY_TITLE_INFO* ti, const std::string & path, const struct stat * statbuf, void * buf, fuse_fill_dir_t filler, bool is_main_title, bool full_title, uint32_t title_idx, uint32_t chapter_idx)
{
    ...
    std::string filenameX(title_buf);

    LPVIRTUALFILE virtualfile = nullptr;
    if (!params.m_format[0].is_multiformat())
    {
        virtualfile = insert_file(VIRTUALTYPE_BLURAY, path + filenameX, statbuf);
    }
    else
    {
        virtualfile = insert_dir(VIRTUALTYPE_BLURAY, path + filenameX, statbuf);
    }

    if (virtualfile == nullptr)
    {
        Logging::error(path, "Failed to create virtual path: %1", (path + filenameX).c_str());
        errno = EIO;
        return false;
    }
    ...
    if (!transcoder_cached_filesize(virtualfile, &virtualfile->m_st))
    {
        ...
        Logging::trace(virtualfile->m_origfile, "Video %1 %2x%3@%<%5.2f>4%5 fps %6 [%7]", format_bitrate(video_bit_rate).c_str(), width, height, av_q2d(framerate), interleaved ? "i" : "p", format_size(size).c_str(), format_duration(duration).c_str());
        if (audio > -1)
        {
            Logging::trace(virtualfile->m_origfile, "Audio %1 channels %2", channels, format_samplerate(sample_rate).c_str());
        }
...
static bool create_cuesheet_virtualfile(Track *track, int titleno, const std::string & filename, const std::string & path, const struct stat * statbuf)
{
    ...
    if (!params.m_format[0].is_multiformat())
    {
        virtualfile = insert_file(VIRTUALTYPE_DISK, path + virtfilename, statbuf, VIRTUALFLAG_CUESHEET);
    }
    else
    {
