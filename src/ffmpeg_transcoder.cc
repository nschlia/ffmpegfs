...
int FFMPEG_Transcoder::open_input_file(const char* filename)
{
    ...
    if (is_open())
    {
        ffmpegfs_warning("%s * Already open.", filename);
        return 0;
    }
    ...
    if (ret < 0)
    {
        ffmpegfs_error("%s * Could not open input file (error '%s').", filename, ffmpeg_geterror(ret).c_str());
        return ret;
    }
    ...
    {
        ffmpegfs_error("%s * Option %s not found.", filename, t->key);
    ...
    {
        ffmpegfs_error("%s * Failed to open video codec (error '%s').", filename, ffmpeg_geterror(ret).c_str());
        return ret;
    }
...
#warning "Your FFMPEG distribution is missing AV_CODEC_CAP_TRUNCATED flag. Probably requires fixing!"
    ...
    {
        ffmpegfs_error("%s * File contains neither a video nor an audio stream.", filename);
...
int FFMPEG_Transcoder::open_output_file(Buffer *buffer)
{
    ...
    {
        ffmpegfs_error("%s * Out of memory pre-allocating buffer.", destname());
...
int FFMPEG_Transcoder::add_stream(AVCodecID codec_id)
{
    ...
    if (!output_codec)
    {
        ffmpegfs_error("%s * Could not find encoder '%s'.", destname(), avcodec_get_name(codec_id));
        return AVERROR(EINVAL);
    }
    ...
    if (!output_stream)
    {
        ffmpegfs_error("%s * Could not allocate stream for encoder '%s'.", destname(), avcodec_get_name(codec_id));
        return AVERROR(ENOMEM);
    }
    ...
    case AVMEDIA_TYPE_AUDIO:
    {
        ...
        if (get_output_bit_rate(m_in.m_audio.m_pStream, params.m_audiobitrate, &output_codec_ctx->bit_rate))
        {
            // Limit sample rate
        ...
        if (get_output_sample_rate(m_in.m_audio.m_pStream, params.m_audiosamplerate, &output_codec_ctx->sample_rate))
        {
            // Limit sample rate
    ...
    case AVMEDIA_TYPE_VIDEO:
    {
        ...
        if (get_output_bit_rate(m_in.m_video.m_pStream, params.m_videobitrate, &output_codec_ctx->bit_rate))
        {
            // Limit sample rate
...
#if LAVF_DEP_AVSTREAM_CODEC
        output_codec_ctx->width                     = m_in.m_video.m_pStream->codecpar->width;
        output_codec_ctx->height                    = m_in.m_video.m_pStream->codecpar->height;
#else
        output_codec_ctx->width                     = m_in.m_video.m_pStream->codec->width;
        output_codec_ctx->height                    = m_in.m_video.m_pStream->codec->height;
#endif
...
#if LAVF_DEP_AVSTREAM_CODEC
        output_codec_ctx->sample_aspect_ratio       = m_in.m_video.m_pStream->codecpar->sample_aspect_ratio;
#else
        output_codec_ctx->sample_aspect_ratio       = m_in.m_video.m_pStream->codec->sample_aspect_ratio;
#endif

#if LAVF_DEP_AVSTREAM_CODEC
        if (m_in.m_video.m_pStream->codecpar->format != output_codec_ctx->pix_fmt ||
                m_in.m_video.m_pStream->codecpar->width != output_codec_ctx->width ||
                m_in.m_video.m_pStream->codecpar->height != output_codec_ctx->height)
#else
        if (m_in.m_video.m_pStream->codec->pix_fmt != output_codec_ctx->pix_fmt ||
                m_in.m_video.m_pStream->codec->width != output_codec_ctx->width ||
                m_in.m_video.m_pStream->codec->height != output_codec_ctx->height)
#endif
        {
            // Rescale image if required
#if LAVF_DEP_AVSTREAM_CODEC
            const AVPixFmtDescriptor *fmtin = av_pix_fmt_desc_get((AVPixelFormat)m_in.m_video.m_pStream->codecpar->format);
#else
            const AVPixFmtDescriptor *fmtin = av_pix_fmt_desc_get((AVPixelFormat)m_in.m_video.m_pStream->codec->pix_fmt);
#endif
            ...
            m_pSws_ctx = sws_getContext(
                        // Source settings
            #if LAVF_DEP_AVSTREAM_CODEC
                        m_in.m_video.m_pStream->codecpar->width,                  // width
                        m_in.m_video.m_pStream->codecpar->height,                 // height
                        (AVPixelFormat)m_in.m_video.m_pStream->codecpar->format,  // format
            #else
                        m_in.m_video.m_pStream->codec->width,                     // width
                        m_in.m_video.m_pStream->codec->height,                    // height
                        m_in.m_video.m_pStream->codec->pix_fmt,                   // format
            #endif
                        // Target settings
                        output_codec_ctx->width,                                // width
                        output_codec_ctx->height,                               // height
            ...
            if (!m_pSws_ctx)
            {
                ffmpegfs_error("%s * Could not allocate scaling/conversion context.", destname());
                return AVERROR(ENOMEM);
            }
        }
    ...
    ffmpegfs_debug("%s * Successfully opened %s output codec %s.", destname(), get_media_type_string(output_codec->type), get_codec_name(codec_id));
...
int FFMPEG_Transcoder::open_output_filestreams(Buffer *buffer)
{
    ...
    if (format == NULL)
    {
        ffmpegfs_error("%s * Unknown format type '%s'.", filename(), params.m_desttype);
        return -1;
    }

    ffmpegfs_debug("%s * Opening format type '%s'.", filename(), params.m_desttype);

    // Create a new format context for the output container format.
    avformat_alloc_output_context2(&m_out.m_pFormat_ctx, NULL, format, NULL);
    if (!m_out.m_pFormat_ctx)
    {
        ffmpegfs_error("%s * Could not allocate output format context.", destname());
        return AVERROR(ENOMEM);
    }
...
int FFMPEG_Transcoder::init_fifo()
{
    ...
    {
        ffmpegfs_error("%s * Could not allocate FIFO.", destname());
        return AVERROR(ENOMEM);
    }
    return 0;
}
...
AVFrame *FFMPEG_Transcoder::alloc_picture(AVPixelFormat pix_fmt, int width, int height)
{
    ...
    if (ret < 0)
    {
        ffmpegfs_error("%s * Could not allocate frame data.", destname());
...
int FFMPEG_Transcoder::init_converted_samples(uint8_t ***converted_input_samples, int frame_size)
{
    ...
    {
        ffmpegfs_error("%s * Could not allocate converted input sample pointers.", destname());
        return AVERROR(ENOMEM);
    }
    ...
    {
        ffmpegfs_error("%s * Could not allocate converted input samples (error '%s').", destname(), ffmpeg_geterror(ret).c_str());
        av_freep(&(*converted_input_samples)[0]);
        free(*converted_input_samples);
        return ret;
    }
...
int FFMPEG_Transcoder::add_samples_to_fifo(uint8_t **converted_input_samples, const int frame_size)
{
    ...
    {
        ffmpegfs_error("%s * Could not reallocate FIFO.", destname());
        return ret;
    }
    ...
    if (ret < frame_size)
    {
        if (ret < 0)
        {
            ffmpegfs_error("%s * Could not write data to FIFO (error '%s').", destname(), ffmpeg_geterror(ret).c_str());
        }
        else
        {
            ffmpegfs_error("%s * Could not write data to FIFO.", destname());
            ret = AVERROR_EXIT;
        }
        return AVERROR_EXIT;
    }
...
int FFMPEG_Transcoder::read_decode_convert_and_store(int *finished)
{
    ...
    try
    {
        ...
        if (ret < 0)
        {
            ...
            {
                ffmpegfs_error("%s * Could not read frame (error '%s').", filename(), ffmpeg_geterror(ret).c_str());
                throw ret;
            }
        }
...
int FFMPEG_Transcoder::init_audio_output_frame(AVFrame **frame, int frame_size)
{
    ...
    {
        ffmpegfs_error("%s * Could not allocate output frame.", destname());
        return AVERROR_EXIT;
    }
    ...
    {
        ffmpegfs_error("%s * Could allocate output frame samples (error '%s').", destname(), ffmpeg_geterror(ret).c_str());
        av_frame_free(frame);
        return ret;
    }
...
int FFMPEG_Transcoder::encode_video_frame(AVFrame *frame, int *data_present)
{
    ...
    {
#if LAVF_DEP_AVSTREAM_CODEC
        //if (m_out.m_video.m_pCodec_ctx->flags & (AV_CODEC_FLAG_INTERLACED_DCT | AV_CODEC_FLAG_INTERLACED_ME) && ost->top_field_first >= 0)
        ...
        if (*data_present)
        {
            ...
            if (!(m_out.m_pFormat_ctx->oformat->flags & AVFMT_NOTIMESTAMPS))
            {
                        ...
                        pkt.dts > pkt.pts)
                {
                    ...
                    ffmpegfs_warning("%s * Invalid DTS: %" PRId64 " PTS: %" PRId64 " in video output, replacing by guess.", destname(), pkt.dts, pkt.pts);
                ...
                if (pkt.dts != (int64_t)AV_NOPTS_VALUE && m_out.m_last_mux_dts != (int64_t)AV_NOPTS_VALUE)
                {
                    ...
                    if (pkt.dts < max)
                    {
                        ffmpegfs_warning("%s * Non-monotonous DTS in video output stream; previous: %" PRId64 ", current: %" PRId64 "; changing to %" PRId64 ". This may result in incorrect timestamps in the output.", destname(), m_out.m_last_mux_dts, pkt.dts, max);
            ...
            if (ret < 0)
            {
                ffmpegfs_error("%s * Could not write video frame (error '%s').", destname(), ffmpeg_geterror(ret).c_str());
                av_packet_unref(&pkt);
                return ret;
            }
        }
...
int FFMPEG_Transcoder::load_encode_and_write(int frame_size)
{
    ...
    if (ret < frame_size)
    {
        if (ret < 0)
        {
            ffmpegfs_error("%s * Could not read data from FIFO (error '%s').", destname(), ffmpeg_geterror(ret).c_str());
        }
        else
        {
            ffmpegfs_error("%s * Could not read data from FIFO.", destname());
            ret = AVERROR_EXIT;
        }
...
#if LAVF_DEP_AVSTREAM_CODEC
    if (m_in.m_audio.m_pStream != NULL && m_in.m_audio.m_pStream->codecpar->codec_id == AV_CODEC_ID_VORBIS)
#else
    if (m_in.m_audio.m_pStream != NULL && m_in.m_audio.m_pStream->codec->codec_id == AV_CODEC_ID_VORBIS)
#endif
    {
...
size_t FFMPEG_Transcoder::calculate_size()
{
    if (m_nCalculated_size == 0 && m_in.m_pFormat_ctx != NULL)
    {
        ...
        {
#if !defined(USING_LIBAV) && (LIBAVUTIL_VERSION_MAJOR > 54)
            int64_t audiobitrate = m_in.m_audio.m_pCodec_ctx->bit_rate;
#else // USING_LIBAV
            int audiobitrate = m_in.m_audio.m_pCodec_ctx->bit_rate;
#endif

            get_output_bit_rate(m_in.m_audio.m_pStream, params.m_audiobitrate, &audiobitrate);
            ...
            {
                int nChannels = 2; //m_in.m_audio.m_pCodec_ctx->channels;
                int nBytesPerSample =  2; // av_get_bytes_per_sample(m_in.m_audio.m_pCodec_ctx->sample_fmt);
                int nSampleRate = m_in.m_audio.m_pCodec_ctx->sample_rate;

                get_output_sample_rate(m_in.m_audio.m_pStream, params.m_audiosamplerate, &nSampleRate);
            ...
            default:
            {
                ffmpegfs_error("%s * Internal error - unsupported audio codec '%s' for format %s.", filename(), get_codec_name(audio_codec_id), params.m_desttype);
                break;
            }
            }
        }
        ...
        if (m_in.m_video.m_nStream_idx > -1)
        {
            ...
            {
#if !defined(USING_LIBAV) && (LIBAVUTIL_VERSION_MAJOR > 54)
                int64_t videobitrate = m_in.m_video.m_pCodec_ctx->bit_rate;
#else // USING_LIBAV
                int videobitrate = m_in.m_video.m_pCodec_ctx->bit_rate;
#endif
                int bitrateoverhead = 0;

                get_output_bit_rate(m_in.m_video.m_pStream, params.m_videobitrate, &videobitrate);
...
int FFMPEG_Transcoder::encode_finish()
{
    ...
    if (ret < 0)
    {
        ffmpegfs_error("%s * Error writing trailer (error '%s').", destname(), ffmpeg_geterror(ret).c_str());
    }
...
void FFMPEG_Transcoder::close()
{
    ...
    if (bClosed)
    {
        ...
        {
            ffmpegfs_warning("%s * %i audio samples left in buffer and not written to target file!", destname(), nAudioSamplesLeft);
        }

        if (nVideoFramesLeft)
        {
            ffmpegfs_warning("%s * %zu video frames left in buffer and not written to target file!", destname(), nVideoFramesLeft);
        }
...
const string & FFMPEG_Transcoder::get_destname(string *destname, const string & filename)
{
