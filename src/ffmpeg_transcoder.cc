...
int FFMPEG_Transcoder::add_stream(AVCodecID codec_id)
{
    ...
    if (!output_stream)
    {
        ffmpegfs_error("%s * Could not allocate stream.", destname());
        return AVERROR(ENOMEM);
    }
...
int FFMPEG_Transcoder::encode_audio_frame(AVFrame *frame, int *data_present)
{
    ...
    *data_present = 0;
    ...
    ret = avcodec_send_frame(m_out.m_pAudio_codec_ctx, frame);
    ...
    while (ret >= 0)
    {
        *data_present = 0;
        ...
        ret = avcodec_receive_packet(m_out.m_pAudio_codec_ctx, &pkt);
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
        {
            av_packet_unref(&pkt);
            return ret;
...
int FFMPEG_Transcoder::encode_video_frame(AVFrame *frame, int *data_present)
{
    ...
    *data_present = 0;
    ...
    // send the frame for encoding
    ret = avcodec_send_frame(m_out.m_pVideo_codec_ctx, frame);
    if (ret < 0 && ret != AVERROR_EOF)
    {
        ffmpegfs_error("%s * Could not encode video frame (error '%s').", destname(), ffmpeg_geterror(ret).c_str());
        av_packet_unref(&pkt);
    ...
    while (ret >= 0)
    {
        *data_present = 0;
        ...
        ret = avcodec_receive_packet(m_out.m_pVideo_codec_ctx, &pkt);
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
        {
            av_packet_unref(&pkt);
            return ret;
